...
tests =
    ...
    , ( "two"
      , testRegex two
            ...
            , [C,A,B,A,A,C,B,B,C,A]
            , [B,C,B]
            , [A,A,B,A,A,B,C,A,A,C,B,B,B,B,C,C,C,A,A,C,C,C,A,A,C,B,C,A,A,C,C,B,C,A,A,B,C,B,C,C,C] -- added
            ]
            [ [A]
            , [C,A,B,B,C,B]
            , [A,B,C,A,B,C,A,B,C]
    ...
    , ( "matchEmpty"
      , [ test "e" (matchEmpty (Empty :: RE ABC)) True
        ...
        , test "(A*e)" (matchEmpty (Repeat a :* Empty)) True -- added
        , test "(A+C)" (matchEmpty (Plus a :+ c)) False -- added
        , test "(A*|C)" (matchEmpty (Repeat a :+ c)) True --added
        , test "0" (matchEmpty Never) False
        , test "A|0" (matchEmpty (a :+ Never)) False
        , test "eA*" (matchEmpty (Empty :+ Repeat a)) True -- this works for me
        ]
      )
    , ( "firsts"
